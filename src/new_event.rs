...
use std::{fmt::Debug, hash::Hash, usize};

use anyhow::{bail, Context};

use crate::{event_worker::EventWorker, message::Message};
...
impl<I, T> SubscribeOption<I, T> {
    ...
    pub fn send(&self, event: Event<I, T>) -> anyhow::Result<()> {
        match (self, event) {
            ...
            (
                SubscribeOption::MessageRecived(_)
                | SubscribeOption::MessageSent(_)
                | SubscribeOption::Error(_),
                _,
...
mod tests {
    ...
    async fn wait_test() {
        ...
        dbg!(
            tokio::time::timeout(std::time::Duration::from_millis(10), async move {
                dbg!(recv.recv().await);
            })
            .await
        )
